const mongoose = require("mongoose")

const username = "El-turco"
const password = encodeURIComponent("12345emre")

const db = "BasketService"

const SecretKey ="";


const Connect = mongoose.connect("mongodb+srv://galatasaray22100:12345emre@basketservice.fzh1a8g.mongodb.net/BasketService")

.then(() => {console.log("Conection mongo db " + db + Connect)}).catch((err) => {console.log(err)})


module.exports = {Connect,SecretKey,db}